so-net-u-page-plus-grab
=============

More information about the archiving project can be found on the ArchiveTeam wiki: [So-Net U-Page+](https://www.archiveteam.org/index.php?title=So-Net_U-Page%2B)
...
Package `libzstd-dev` version 1.4.4 is required which is currently available from `buster-backports`.
    ...
    python3 -m pip install setuptools wheel
    python3 -m pip install --upgrade seesaw zstandard requests
    su -c "cd /home/archiveteam; git clone https://github.com/ArchiveTeam/so-net-u-page-plus-grab.git; cd so-net-u-page-plus-grab; ./get-wget-lua.sh" archiveteam
    screen su -c "cd /home/archiveteam/so-net-u-page-plus-grab/; run-pipeline3 pipeline.py --concurrent 2 --address '127.0.0.1' YOURNICKHERE" archiveteam
    [... ctrl+A D to detach ...]
...
**There is a known issue with some packaged versions of rsync. If you get errors during the upload stage, so-net-u-page-plus-grab will not work with your rsync version.**
...
6. `screen su -c "cd /home/archiveteam/so-net-u-page-plus-grab/; run-pipeline pipeline.py --concurrent 2 --address '127.0.0.1' YOURNICKHERE" archiveteam`
...
### For Alpine Linux:
    ...
    git clone https://github.com/ArchiveTeam/so-net-u-page-plus-grab
    cd so-net-u-page-plus-grab; ./get-wget-lua.sh
